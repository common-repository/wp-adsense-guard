...
if(!function_exists( 'wpadgu_geoip_load_shared_mem' )){
	function wpadgu_geoip_load_shared_mem($file)
	{
	    $fp = fopen($file, "rb");
	    if (!$fp) {
...
if(!function_exists('wpadgu_setup_segments')){
function wpadgu_setup_segments($gi)
{
...
if(!function_exists( 'wpadgu_sharedMemRead' )){
	function wpadgu_sharedMemRead($gi, $start, $maxLength)
	{
...
if(!function_exists( 'wpadgu_geoip_open' )){
	function wpadgu_geoip_open($filename, $flags)
	{
	    $gi = new wpadgu_GeoIP;
	    $gi->flags = $flags;
	    if ($gi->flags & GEOIP_SHARED_MEMORY) {
	    ...
	    $gi = wpadgu_setup_segments($gi);
	    return $gi;
	}
}

if(!function_exists( 'wpadgu_geoip_close' )){
function wpadgu_geoip_close($gi)
{
    if ($gi->flags & GEOIP_SHARED_MEMORY) {
        return true;
    }
...
if(!function_exists('wpadgu_geoip_country_id_by_name_v6')){
	function wpadgu_geoip_country_id_by_name_v6($gi, $name)
	{
	    ...
	    return wpadgu_geoip_country_id_by_addr_v6($gi, $addr);
	}
}

if(!function_exists('wpadgu_geoip_country_id_by_name')){
	function wpadgu_geoip_country_id_by_name($gi, $name)
	{
	    $addr = gethostbyname($name);
	    if (!$addr || $addr == $name) {
	        return false;
	    }
	    return wpadgu_geoip_country_id_by_addr($gi, $addr);
	}
}

if(!function_exists('wpadgu_geoip_country_code_by_name_v6')){
	function wpadgu_geoip_country_code_by_name_v6($gi, $name)
	{
	    $country_id = wpadgu_geoip_country_id_by_name_v6($gi, $name);
	    if ($country_id !== false) {
...
if(!function_exists('wpadgu_geoip_country_code_by_name')){
	function wpadgu_geoip_country_code_by_name($gi, $name)
	{
	    $country_id = wpadgu_geoip_country_id_by_name($gi, $name);
	    if ($country_id !== false) {
	        return $gi->GEOIP_COUNTRY_CODES[$country_id];
	    }
	    return false;
...
if(!function_exists('wpadgu_geoip_country_name_by_name_v6')){
	function wpadgu_geoip_country_name_by_name_v6($gi, $name)
	{
	    $country_id = wpadgu_geoip_country_id_by_name_v6($gi, $name);
	    if ($country_id !== false) {
...
if(!function_exists('wpadgu_geoip_country_name_by_name')){
	function wpadgu_geoip_country_name_by_name($gi, $name)
	{
	    $country_id = wpadgu_geoip_country_id_by_name($gi, $name);
	    if ($country_id !== false) {
	        return $gi->GEOIP_COUNTRY_NAMES[$country_id];
...
if(!function_exists('wpadgu_geoip_country_id_by_addr_v6')){
	function wpadgu_geoip_country_id_by_addr_v6($gi, $addr)
	{
	    $ipnum = inet_pton($addr);
	    return wpadgu_geoip_seek_country_v6($gi, $ipnum) - GEOIP_COUNTRY_BEGIN;
	}
}

if(!function_exists('wpadgu_geoip_country_id_by_addr')){
function wpadgu_geoip_country_id_by_addr($gi, $addr)
{
    $ipnum = ip2long($addr);
    return wpadgu_geoip_seek_country($gi, $ipnum) - GEOIP_COUNTRY_BEGIN;
}
}

if(!function_exists('wpadgu_geoip_country_code_by_addr_v6')){
function wpadgu_geoip_country_code_by_addr_v6($gi, $addr)
{
    $country_id = wpadgu_geoip_country_id_by_addr_v6($gi, $addr);
    if ($country_id !== false) {
        return $gi->GEOIP_COUNTRY_CODES[$country_id];
    }
    return false;
...
if(!function_exists('wpadgu_geoip_country_code_by_addr')){
function wpadgu_geoip_country_code_by_addr($gi, $addr)
{
    if ($gi->databaseType == GEOIP_CITY_EDITION_REV1) {
        ...
        $country_id = wpadgu_geoip_country_id_by_addr($gi, $addr);
        if ($country_id !== false) {
            return $gi->GEOIP_COUNTRY_CODES[$country_id];
        }
    }
...
if(!function_exists('wpadgu_geoip_country_name_by_addr_v6')){
function wpadgu_geoip_country_name_by_addr_v6($gi, $addr)
{
    $country_id = wpadgu_geoip_country_id_by_addr_v6($gi, $addr);
    if ($country_id !== false) {
        return $gi->GEOIP_COUNTRY_NAMES[$country_id];
...
if(!function_exists('wpadgu_geoip_country_name_by_addr')){
function wpadgu_geoip_country_name_by_addr($gi, $addr)
{
    if ($gi->databaseType == GEOIP_CITY_EDITION_REV1) {
        $record = GeoIP_record_by_addr($gi, $addr);
...
if(!function_exists('wpadgu_geoip_seek_country_v6')){
function wpadgu_geoip_seek_country_v6($gi, $ipnum)
{
    ...
    for ($depth = 127; $depth >= 0; --$depth) {
        if ($gi->flags & GEOIP_MEMORY_CACHE) {
            $buf = wpadgu_safe_substr(
                $gi->memory_buffer,
                2 * $gi->record_length * $offset,
                2 * $gi->record_length
            );
        } elseif ($gi->flags & GEOIP_SHARED_MEMORY) {
            $buf = wpadgu_sharedMemRead($gi,
                2 * $gi->record_length * $offset,
                2 * $gi->record_length
            );
        } else {
...
if(!function_exists('wpadgu_geoip_seek_country')){
function wpadgu_geoip_seek_country($gi, $ipnum)
{
    $offset = 0;
...
if(!function_exists('wpadgu_common_get_org')){
function wpadgu_common_get_org($gi, $seek_org)
{
    $record_pointer = $seek_org + (2 * $gi->record_length - 1) * $gi->databaseSegments;
    if ($gi->flags & GEOIP_SHARED_MEMORY) {
        $org_buf = wpadgu_sharedMemRead($gi, $record_pointer, MAX_ORG_RECORD_LENGTH);
    } else {
        fseek($gi->filehandle, $record_pointer, SEEK_SET);
        $org_buf = fread($gi->filehandle, MAX_ORG_RECORD_LENGTH);
    }
    $org_buf = wpadgu_safe_substr($org_buf, 0, strpos($org_buf, "\0"));
    return $org_buf;
}
}

if(!function_exists('wpadgu_get_org_v6')){
function wpadgu_get_org_v6($gi, $ipnum)
{
    $seek_org = wpadgu_geoip_seek_country_v6($gi, $ipnum);
    if ($seek_org == $gi->databaseSegments) {
        return null;
    }
    return wpadgu_common_get_org($gi, $seek_org);
}
}

if(!function_exists('wpadgu_get_org')){
function wpadgu_get_org($gi, $ipnum)
{
    $seek_org = wpadgu_geoip_seek_country($gi, $ipnum);
    if ($seek_org == $gi->databaseSegments) {
        return null;
    }
    return wpadgu_common_get_org($gi, $seek_org);
...
if(!function_exists('wpadgu_geoip_name_by_addr_v6')){
function wpadgu_geoip_name_by_addr_v6($gi, $addr)
{
    if ($addr == null) {
        return 0;
    }
    $ipnum = inet_pton($addr);
    return wpadgu_get_org_v6($gi, $ipnum);
}
}

if(!function_exists('wpadgu_geoip_name_by_addr')){
function wpadgu_geoip_name_by_addr($gi, $addr)
{
    if ($addr == null) {
        return 0;
    }
    $ipnum = ip2long($addr);
    return wpadgu_get_org($gi, $ipnum);
}
}

function wpadgu_geoip_org_by_addr($gi, $addr)
{
    return wpadgu_geoip_name_by_addr($gi, $addr);
}

function wpadgu_get_region($gi, $ipnum)
{
    if ($gi->databaseType == GEOIP_REGION_EDITION_REV0) {
        $seek_region = wpadgu_geoip_seek_country($gi, $ipnum) - GEOIP_STATE_BEGIN_REV0;
        if ($seek_region >= 1000) {
            $country_code = "US";
        ...
        $seek_region = wpadgu_geoip_seek_country($gi, $ipnum) - GEOIP_STATE_BEGIN_REV1;
...
function geoip_region_by_addr($gi, $addr)
{
    ...
    $ipnum = ip2long($addr);
    return wpadgu_get_region($gi, $ipnum);
}
}

function wpadgu_safe_substr($string, $start, $length)
{
